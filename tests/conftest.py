...
def knowledge_provider() -> MockKnowledgeProvider:
    ...
    provider.add(
        "SDS Upload Pipeline",
        "Safety Data Sheets must be uploaded in PDF format. CAS numbers are validated against PubChem.",
        source="outline",
    )
    provider.add(
        "GHS Classification",
        "GHS pictograms are assigned based on hazard category. Signal words: Danger or Warning.",
        source="outline",
    )
    return provider
    ...
    return MockLLMProvider(
        {
            ...
            "reflex.uc-quality-check": '{"valid": true, "violations": [], "warnings": [], "adr_conflicts": []}',
